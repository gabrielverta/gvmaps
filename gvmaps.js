(function(){
    ...
    GVMap.prototype.query = function(query, obj){
		...
		obj.geocoder.geocode({address: query}, function(results, status){
			if (status == google.maps.GeocoderStatus.OK) {
				...
				if(gmap.params.icon != null){
					marker_opts.icon = gmap.params.icon
				}
				if(gmap.params.draggable){
					marker_opts.draggable = gmap.params.draggable
				}
				...
				if(gmap.params.draggable){
					google.maps.event.addListener(marker, "dragstart", function(){
						if("undefined" != typeof(obj.map.infowindow)){
							obj.map.infowindow.close()
						}
	                })
	                google.maps.event.addListener(marker, "dragend", function(){
	                    gmap.ondrop(obj, marker)
	                })
				}
			} else {
    ...
    GVMap.prototype.route = function(from, to, obj){
        ...
        var request = {
		    origin: from, 
		    destination: to,
		    travelMode: GVMap.option.travelMode,
			unitSystem: GVMap.option.unitSystem
		}
		if(jQuery(obj).attr('data-panel') != null){
			obj.directions.setPanel(jQuery("#" + jQuery(obj).attr('data-panel'))[0])
		}
		directionsService.route(request, function(result, status){
			if (status == google.maps.DirectionsStatus.OK) {
			    obj.directions.setDirections(result);
			} else {
    ...
    GVMap.prototype.getMapType = function(el){
        ...
        switch(type){
            case 'normal': return google.maps.MapTypeId.ROADMAP;
    ...
    GVMap.prototype.getControl = function(el){
        ...
        switch(control){
            ...
            case 'large': return google.maps.NavigationControlStyle.DEFAULT;
    ...
    GVMap.prototype.links = function(selector){
        onEvent = 'click'
        var gmap = this
        ...
        jQuery(selector).bind(onEvent, function(e){
			jQuery('.gvmap-selected').removeClass('gvmap-selected')
